...
class Controller:
    ...
    def __init__(self, name: str, **kwargs) -> None:
        if not kwargs:
            controller_data = get_controller_data()
            if name not in controller_data:
                raise ValueError(f"Invalid controller: {name}")
            kwargs = controller_data[name]
        self.name: str = name
        self.buttons: dict[int, str] = defaultdict(str, kwargs["buttons"])
        self.axis_buttons: dict[int, str] = defaultdict(str, kwargs["axis_buttons"])
        self.axes: dict[int, str] = defaultdict(str, kwargs["axes"])
        self.num_buttons: int = kwargs["num_buttons"]
        self.num_axes: int = kwargs["num_axes"]
        self.has_stick: bool = kwargs["has_stick"]
        self.has_dpad: bool = kwargs["has_dpad"]
        self.dpad_buttons = self.get_dpad_buttons()
        self.stick_button = self.get_stick_button()
        self.supported: bool = kwargs["supported"]
        self.is_custom = "is_custom" in kwargs and kwargs["is_custom"]
        self.parent = Controller(kwargs["parent"]) if self.is_custom else self

    @staticmethod
    def from_dict(data: dict[str, Any]) -> Controller:
    ...
    def to_toml(self) -> str:
        """Converts the controller to a TOML string."""
        doc = tomlkit.document()
        doc.add(tomlkit.comment("This file was generated by Contanki. Do not edit."))
        doc["name"] = self.name
        doc["num_buttons"] = self.num_buttons
        doc["num_axes"] = self.num_axes
        doc["has_stick"] = self.has_stick
        doc["has_dpad"] = self.has_dpad
        doc["supported"] = self.supported
        doc["is_custom"] = self.is_custom
        doc["parent"] = self.parent.name

        def add_table(table: str, data: dict[Any, Any]) -> None:
            _table = tomlkit.table()
            for key, value in data.items():
                _table.add(str(key), value)
            doc.add(table, _table)

        add_table("buttons", self.buttons)
        add_table("axes", self.axes)
        add_table("axis_buttons", self.axis_buttons)

        return doc.as_string()

    @staticmethod
    def from_toml(toml: str) -> Controller:
        """Creates a controller from a TOML string."""
        return Controller.from_dict(int_keys(tomlkit.loads(toml)))
